<?php
/**
 * Template hiển thị chi tiết bài viết Sự kiện năm (CPT: su_kien_nam)
 */
get_header(); ?>
...
<div class="news-detail-wrapper">
    ...
    // 2. Tự động lấy slug trang Tin tức theo ngôn ngữ hiện tại từ từ điển languages.php
    $news_slug = adtec_get_label('slug_tintuc');

    // 3. Tạo link trang Tin tức chuẩn đa ngôn ngữ (Ví dụ: .../en/news/ hoặc .../ja/news-ja/)
    $lang_news_url = function_exists('pll_home_url') ? pll_home_url() . $news_slug . '/' : home_url('/' . $news_slug . '/');
    ...
    $custom_breadcrumbs = array(
        ...
        array(
            'title' => adtec_get_label('tin_tuc_su_kien'), // Nếu có key này trong languages.php
            'url'   => $lang_news_url
        ),
        array(
    ...
    if ( have_posts() ) :
        while ( have_posts() ) : the_post(); 
            ...
            $raw_gallery = rwmb_meta( 'news_gallery', array( 'size' => 'large' ) );
